...
from utils.queries import inventory_query, orders_query, execute_query
from utils.data import extract_inventory_total, extract_orders, remove_tiktok_shop_dups
...
# date filters
end_filter = todays_date.strftime('%Y-%m-%d')
start_filter = (today_dt - timedelta(days=1)).date().strftime('%Y-%m-%d')

orders_query = orders_query.format(start_date=start_filter, end_date=end_filter)
...
inventory_results = execute_query(inventory_query)
...
orders = pd.read_csv('/Users/cspineda/Documents/vp/apps/inventory-forecaster/adc-inventory-tracker/orders_jan1_apr30.csv')
